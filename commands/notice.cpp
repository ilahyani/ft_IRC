...
/*   Updated: 2023/06/02 16:33:15 by kid-bouh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../server.hpp"

void server::notice(std::vector<std::string> params, std::map<int, client>::iterator cl) {
    if (params.size() < 2)
    {
    ...
    if (params[1][0] == '#' && params[1].size() > 1)
    {
        ...
        send_msg_to_clients_who_in_channel(ch, cl->second, "NOTICE " + ch->getName() + " :" + params[2]);
    }
    else
    {
        ...
        sendToClientById(receiver->getsocket(), cl->second, "NOTICE " + receiver->getNickname() + " :" + params[2]);
    }
}